...
TESTING blkfidpool reuse
T{ fid1 blkfidpool-free -> }T
T{ blkfidpool-get VALUE newfid1 -> }T
T{ fid1 newfid1 = -> TRUE }T
T{ fid2 newfid1 = -> FALSE }T

TESTING blkfidpool allocation
T{ blkfidpool-get VALUE fid3 -> }T
T{ fid3 0 = -> FALSE }T
T{ fid1 fid3 = -> FALSE }T
T{ fid2 fid3 = -> FALSE }T

CREATE read-buffer 128 ALLOT


TESTING OPEN-BLKFILE READ-FILE and CLOSE-BLKFILE
...
TESTING OPEN-LIMIT-BLKFILE READ-FILE and CLOSE-BLKFILE
read-buffer 128 0 FILL
...
TESTING OPEN-FENCE-BLKFILE READ-LINE and CLOSE-BLKFILE
read-buffer 128 0 FILL
T{ ipsum-file ipsum-file 1+
          R/O OPEN-FENCE-BLKFILE TO fid1 -> }T
T{ fid1 0 = -> FALSE }T
...
TESTING OPEN-BLKFILE WRITE-LINE and CLOSE-BLKFILE
test-block WIPE  FLUSH
T{ test-block  R/W OPEN-BLKFILE TO fid1 -> }T
T{ S" Write line 1"  fid1 WRITE-LINE -> 0 }T
T{ fid1 FILE-POSITION -> 13 S>D 0 }T
T{ S" Write line 2"  fid1 WRITE-LINE -> 0 }T
T{ fid1 FILE-POSITION -> 26 S>D 0 }T
fid1 .BLKFILE
fid1 BLKF>SLICE  .SLICE
T{ fid1 CLOSE-BLKFILE -> 0 }T
...
TESTING OPEN-BLKFILE READ-LINE short buffers
read-buffer 128 0 FILL
T{ ipsum-file R/O OPEN-BLKFILE TO fid1 -> }T
T{ fid1 0 = -> FALSE }T
T{ read-buffer 3 fid1 READ-LINE -> 3 -1 0 }T
T{ fid1 FILE-POSITION  -> 3 S>D 0 }T
fid1 .BLKFILE
fid1 BLKF>SLICE  .SLICE
read-buffer 32 MEMDUMP
T{ read-buffer 0 fid1 READ-LINE  -> 0  -1 0 }T
T{ fid1 FILE-POSITION  -> 3 S>D 0 }T
fid1 .BLKFILE
fid1 BLKF>SLICE  .SLICE
T{ fid1 CLOSE-BLKFILE -> 0 }T

CR .( Finished blkfile_test2 )
