\ in Forth, you do many things on your own. 
\ This word is used to define 2D arrays
...
: NEW-BLOCK ( -- )
   COUNT-FREE-SPACES
   DUP 1+ 0< IF DROP EXIT THEN
   RANDOM GET-FREE-SPACE
   10 RANDOM 0= IF 2 ELSE 1 THEN SWAP !
;

: 2GRID ( a-y a-x b-y b-x -- a-addr b-addr )
   GRID -ROT GRID SWAP ;
...
: TRY-GRAVITY ( did-something-before operator  )
              (    dest-y dest-x other-y other-x -- )
              (            did-something-after operator )
   2GRID ( ... dest-addr other-addr )
   2DUP CAN-GRAVITY IF
      TRUE HAVE-MOVED !
      DUP @ ( ... dest-addr other-addr other-val )
      ROT ( ... other-addr other-val dest-addr ) !
      0 SWAP !
      NIP TRUE SWAP
   ELSE
      2DROP
...
: TRY-LOST? ( lost-before operator dy dx oy ox )
            (     -- lost-after operator )
...
: GRAVITY-ONCE ( move -- success? ) FALSE SWAP ['] TRY-GRAVITY
   MOVEMENT-LOOP ;
: GRAVITY ( move -- )
   BEGIN
...
: MOVE-LOST? ( move -- lost? ) TRUE SWAP ['] TRY-LOST?
   MOVEMENT-LOOP ;
: END-IF-LOST ( -- lost? )
...
: END-IF-WON ( -- )
   NROWS 0 ?DO
      NCOLS 0 ?DO
         J I GRID @ GRID-VALUE 2048 = IF ." You win!"
            CR BYE THEN
      LOOP
   LOOP
;
