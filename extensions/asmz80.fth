...
BASE @ HEX
: DEFER ( "ccc" --- )
\ Defining word to create a word that can be made to execute 
\ an arbitrary
\ Forth word.    
...
' C! DEFER VC! IS VC! \ Vectorize the important words so we can
' C@ DEFER VC@ IS VC@ \ cross assemble and self-assemble using
' !  DEFER V!  IS V!  \ the same code.
' @  DEFER V@  IS V@
' C, DEFER C_ IS C_
' ,  DEFER ,  IS ,
...
\ This is a prefix assembler, so the source code looks much
\ like traditional assembler source, for example INC HL or 
\ LD DE, $4444
\ It is inspired by the 8086 assembler that came with F-PC
\ under MS-DOS. In 1991-1994 I created such an assembler for
\ the 6809. In 2022 I ported the same concept to the Z80.
\ The Z80 is a very un-orthogonal instruction set. For example
\ the opcodes for the LD mnemonic are literally all over the
\ place. Therefore most instrucitons need a customized handler
\ to compute the opcode from the various operand fields. This
\ is in the IHANDLER variable.

\ Each opcode word sets some of the following variable. Words
\ that represent registers or other operands update some of 
\ these words as well.

\ The A; instruction finally assembles the instruction using 
\ these variables.
VARIABLE IXBYTE \  0 when not used DD or FF when instruction 
                \ needs a DD/FF prefix. 1DD or 1FF when a 
                \ dispalcement is also required.
VARIABLE DISPL  \ Displacement for IX instructions.
VARIABLE OPCODE  \ Opcode. FFFF when no instruction, 8-bit
                 \ value for single byte opcdoes, also $CBxx or
                 \ $EDxx for 2 byte opcode
...
VARIABLE ?OPERAND \ Type of operand on the stack 0 = none, 
                  \ 1 = 1 byte,
                  \ 2 = 2 byte, 
                  \ 3 = 1 byte program counter relative.
: NOINSTR \ Clear the instruction related variables.
    FALSE IXBYTE !  ['] NOOP IHANDLER !  FFFF OPCODE !
    FALSE ?OPERAND !  FALSE SRCREG !  FALSE DSTREG !
;
NOINSTR
\ : H. BASE @ HEX SWAP U. BASE ! ;
: A; \ Assemble current instruction and reset instruction 
     \ variables
    IHANDLER @ EXECUTE
    IXBYTE @ IF IXBYTE @ C, THEN
    OPCODE @ FFFF = IF EXIT THEN
    \ ." Assembling instruction " IXBYTE @ H. OPCODE @ H. 
    \       ?OPERAND @ H. SRCREG @ H. DSTREG @ H. CR
    OPCODE @ FF U> IF
    OPCODE @ 8 RSHIFT C, \ Assemble first byte CB or ED of op.
    IXBYTE @ FF U> IF
        DISPL @ C,  \ For CB opcodes displacement between CB
        0 IXBYTE !  \  and main opcode
    THEN
    THEN
...
\ (to select the correct opcode for the operands) and a 
\ defining word. Some instructions have s simple colon 
\ definition instead of the defining word.
...
: JMP-HANDLER
    ...
    \ Get proper value for C (not C register but Carry cond.) 
    DUP 0= IF
    ...
    THEN
;
\ Defining word takes  both opcode (for conditional & 
\ nonconditonal) and operand inputs
: JMPINST
    CREATE , ,
  DOES> >R A; R> DUP @ ?OPERAND ! CELL+ @ OPCODE ! ['] 
    JMP-HANDLER IHANDLER ! ;
1820 3  JMPINST JR
C3C2 2  JMPINST JP
C9C0 0  JMPINST RET
CDC4 2  JMPINST CALL
...
\ EX has only 3 single-byte opcodes, but they are unrelated
\ to one another.
: EX-HANDLER
    CASE DSTREG @
    ...
    CASE DSTREG @ 8 RSHIFT
        01 OF \ Destination is 8-bit register incuding (HL).
            CASE SRCREG @ 8 RSHIFT
                00 OF \ immediate
                    06 DSTREG @ FF AND 8 * + OPCODE ! 
                    1 ?OPERAND !
                ENDOF
                01 OF \ 8 bit register to register
                    40 DSTREG @ FF AND 8 * + SRCREG @ FF AND
                     + OPCODE !
                ENDOF
...
: ARITH-HANDLER
    DSTREG @ 1FF > IF   \ HL,rp 16 bit ops
        OPCODE @ 8 RSHIFT SRCREG @ FF AND  10 * + OPCODE !
        OPCODE @ 3F > IF ED00 OPCODE +! THEN \ ADC and SBC
    ELSE
        SRCREG @ 0= IF  \ A, Immediate
         OPCODE @ FF AND 46  + OPCODE ! 1 ?OPERAND !
        ELSE     \ 8 bit A,Reg
            OPCODE @ FF AND SRCREG @ FF AND + OPCODE !
        THEN
    THEN
;
: ARITH
...
\ This instruction category comes last to prevent AND OR and 
\ XOR from being shadowed by the assembler words.
...
\ This definition of LABEL is only useful when 
\ cross-assemling/metacompiling.
\ In its current form supports no forward references.
\ When cross-assembling it contains a definition in the host 
\ dictionary only, not in the target dictionary.
: LABEL A; HERE CONSTANT ;


\ The following small words define source/destination registers
\ and other operation modes (), and () for direct addressing.
\ Condition codes for jp/jr/call/ret are also included here.
\ These come at the end, primarily to prevent C, from being
\ shadowed by the assembler word.
: SRC-REG
    CREATE ,
  DOES> @ SRCREG ! ;
: DST-REG
...
0101 SRC-REG C
\ This is a dirty trick. When C, is called after an instruction
\ it will act like a DSTREG type word (sets destination 
\ register to C). Outside an instruction it acts as the regular
\ C, word (allot and store 1 byte)
...
: (), 0400 DSTREG ! 2 ?OPERAND ! ; \ Direct addressing 
                                   \   LD addr (), HL
: ()  0400 SRCREG ! 2 ?OPERAND ! ; \ Direct addressing 
                                   \   LD A, addr ()
...
\ These use JR instruction. The only available condition codes 
\ here are:
\ 0= 0<> (for Z flag) and U< U>= (for carry flag)
\ and B--0= for decrement B and test for zero (DJNZ instr.).
: IF >R A; R> C_ >MARK ;
: THEN A; >RESOLVE ;
